...
import { fetchOrderSizes, normalizeOrder, resolveShiftId, getSettingList } from '../../api/services/checkingService';
...
export default function InputListScreen({ navigation, route }) {
 ...
 const { isLandscape } = useOrientation();
  ...
  const canViewInput = canView(GROUP.CHECKING);
  const canCreateAudit = can(GROUP.CHECKING, ACTION.CREATE);
...
const tlsSearchBarStyle = pickStyle(styles.searchBarLarge,
 ...
 const tlsSearchChio= pickStyle(styles.lineChipTextLarge,
  ...
  const handleOrderPress = useCallback(async (order) => {
    ...
    if (!canCreateAudit) {
      Alert.alert('Permission required', "You don't have permission to perform checking.");
      return;
    }
    ...
    try {
      ...
      navigation.navigate('CheckingDetails', {
        user,
        order,
    ...
    <View style={styles.root}>
      ...
      <View style={styles.headerWrap}>
        <SafeAreaView edges={['top']} style={{ backgroundColor: 'transparent' }}>
          ...
          <View style={styles.headerTopRow}>
            <View>
              <Text style={[styles.title,tlsTitleStyle]}>Checking</Text>
            </View>
          </View>
        </SafeAreaView>
