...
import { retrieveEscalated } from '../../api/services/aqlAuditService';
...
// Fields shared across every record_type -> the common "ORDER DETAILS" card.
// [sourceKey, label, fallbackKey?]
...
function splitObjectFields(obj, { skipKeys = [] } = {}) {
  const rows = [];
  const multilineRows = []; // Notes/Comments — always rendered after every other row.
  const arraySections = [];
  ...
  Object.entries(obj ?? {}).forEach(([key, value]) => {
    ...
    if (typeof value === 'object') return; // nested objects are handled by the caller, not here
...
function createExtraStyles(ms, mvs, fs) {
  return StyleSheet.create({
    ...
    arrayRow: {
      flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: mvs(10),
    },
...
// "N items ›" row — tapping it opens the full list in a bottom sheet.
      ...
      {isQualityCheck ? (
        <View style={extraStyles.qcListWrap}>
          {(items ?? []).map((c, idx) => {
            ...
            return (
              <View
                key={`${c?.check_name}-${idx}`}
                style={[extraStyles.qcPill, pass ? extraStyles.qcPillPass : extraStyles.qcPillFail]}
              >
                <Ionicons
                  name={pass ? 'checkmark-circle' : 'close-circle'}
                  size={ms(14)}
                  color={pass ? '#16A34A' : AppColors.error}
                />
                <Text style={[extraStyles.qcPillText, { color: pass ? '#16A34A' : AppColors.error }]}>
                  {c?.check_name}
                </Text>
              </View>
            );
...
export default function AQLOrderDetailsScreen({ navigation, route }) {
  ...
  const [activeArray, setActiveArray] = useState(null); // { key, label, items } | null
  ...
  const { nestedCards, additional } = useMemo(() => {
    ...
    const skipTopLevel = [
      ...
      'status', // also hides it from QC Verification's flat fallback card
    ];
      ...
      <View style={styles.body}>
        ...
        >
          <View style={[{ width: '100%' }, contentWidthStyle]}>
            {/* One card per nested "*_details" object (rework/rejection + tracker updates) */}
