...
export default function RejectionList({ navigation, route }) {
  ...
  const canListTlsAudit = can(GROUP.REJECTION, ACTION.LIST); 
...
const tlsSearchBarStyle = pickStyle(styles.searchBarLarge,
 ...
 const lines = useMemo(
  ...
  // creating a rejection entry on RejectionDetailsScreen) always shows
  // fresh data.
...
const handleScanSuccess = useCallback(async (data) => {
  ...
  try {
    setScannerVisible(false);
     ...
     const checkResult = await rejectionService.checkTlsDevice({ qr_code: scannedTlsId,type:"rejection" });
    ...
    navigation.navigate('RejectionDetailsScreen', {
      user,
      order: orderForAudit,
    ...
    <View style={styles.root}>
      ...
      <View style={styles.headerWrap}>
        <SafeAreaView edges={['top']} style={{ backgroundColor: 'transparent' }}>
          ...
          <View style={styles.headerTopRow}>
            <View>
              <Text style={[styles.title,tlsTitleStyle]}>Rejection</Text>
             </View>
