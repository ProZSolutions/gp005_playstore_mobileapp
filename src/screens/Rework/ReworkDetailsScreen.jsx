...
import reworkService, { mapOrderSizes } from '../../api/services/reworkService';
...
import { rejectionlistDeviceMapping } from '../../api/services/deviceMappingService';
...
function findDeviceMachineRecord(records, scannedId) {
  console.log("Scanned records "+JSON.stringify(records));
...
export default function ReworkDetailsScreen({ navigation, route }) {
  ...
  (async () => {
    ...
    if (!storedLineId) {
      console.warn('ReworkDetailsScreen: no stored lineId available for slot verification');
      return;
    }
    const { slot } = await verifyAndGetSlot({
      lineId: storedLineId,
      navigation,
      listRouteName: 'ReworkListScreen',
    });
  ...
  const loadDeviceMachineRecords = useCallback(async () => {
    ...
    try {
      const result = await rejectionlistDeviceMapping({
        tlsId: 'all',
        machineId: 'all',
        status: 'active',
        page: 1,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('ReworkDetailsScreen: failed to load shift data', e.message);
      }
    })();
    return () => { cancelled = true; };
  }, []);
  ...
  const topDefect = useMemo(() => {
    ...
    console.log(" top defect list "+JSON.stringify(list));
  ...
  const goToOperationDetails = useCallback((scanResult) => {
    console.log("scanned result "+scanResult?.machne_type_name +" machine type "+selectedOperation?.machineType);
    const lineId = Array.isArray(line) ? line[0] : line;
